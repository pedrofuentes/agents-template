...
- **HTML comment reminder** (AGENTS.md Associated Documentation): Projects that adopt release tooling after setup get an inline comment explaining what to adjust.
- **Dim-f generic CHANGELOG check**: Updated to handle both manual and auto-generated changelogs — verifies release inputs (commit messages, changeset files) instead of direct CHANGELOG edits when release tooling is active. Remains project-type-agnostic.
