...
**Finding-class recurrence:** when a re-review yields a new 🔴 of the same finding class (same mechanism family from the same dimension — e.g., another bypass of the same separator/blocklist class) as a 🔴 in any prior cycle of this PR (2nd+ occurrence across Report IDs), the report MUST carry a `Recurrence: <class> × cycle <N>` line directly under `Required action`, recommending a change of approach rather than another patch of the instance.
...
2. **On failure:** Retry once. If still failing, mark ❌ and declare degraded mode. **Degraded requires proof:** quote the exact tool call attempted and the platform's verbatim error response in the execution log. No quoted attempt → REJECTED. A background sub-agent result that has not arrived or cannot be collected via available tools at finalization IS a dispatch failure: re-dispatch that dimension **synchronously** once, then apply this rule — never stall the turn waiting on it, and never self-derive the dimension in its place (self-derivation is degraded mode, §Mode declaration).
...
**Turn-completion gate:** a Sentinel turn may end ONLY by emitting the full report (first line `Status:`) — never a progress note, partial status, or "waiting on dimension X". A dimension result still missing at finalization is handled by Phase 2 §On failure (synchronous re-dispatch once → retry → ❌ degraded with proof); then decide on available evidence.
